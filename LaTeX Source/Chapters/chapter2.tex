...
Downlink messages have to be initiated by the end device, the frequency in use is the same as the first uplink message plus a known delta, there is a delay of twenty seconds between the first transmitted frame and a reception window this one only lasts for a maximum of twenty five seconds~\cite{SIGTECH}.
...
\subsubsection{NB-IoT Devices Comparison} % (fold)
\label{sec:NB_table_sota}
In the following table~\ref{tab:NB-comparison}  was done a comparison between 3 devices using the NB-IoT technology.\\
...
In the following table is presented a comparison between 3 devices using LTE-M technology.\\
\begin{table}[htbp]
\begin{tabular}{@{}lllll@{}}
\toprule
