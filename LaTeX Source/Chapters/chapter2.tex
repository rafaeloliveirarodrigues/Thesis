...
% = Introduction =
% ================
%15 a 30 páginas  deve citar entre 20 a 40 referências.IoT,LoRa
...
The following table~\ref{tab:PwD-comparison} shows a comparison between the above mentioned devices, were the price of the product is presented, and in the technical part, a comparison of the  geolocation technique  and communication method is also shown. By reading this table the wearable type of these solutions are always similar except in the smart sole, and geolocation is always based on ~\gls{GPS}, the communication is always cellular except in the TicTocTracker product were WiFi is also an options. The conclusion is that all the solution are somehow similar, expensive, and not optimized for power efficiency. \newline
\begin{table}[htbp]
\begin{tabular}{lllll}
\hline
Company & GTX & mCare & TicTocTrack & Bluewater \\ \hline
Product & SmartSole & watch & TicTokTracker & Bluewater watch \\
...
\textit{"By the year of 2025, up to 75 billion devices would be connected in Internet-of-Things (\gls{IoT}), with potential economic impact of around 11.1 trillion \$ a year"}~\cite{ LPIkpehai2019}.The need for a~\gls{LPWAN} emerges when M2M communication is necessary and other wireless networks for example the ones with a short range (Bluethoot,RFID, Wifi or Zigbee) are not a good fit for our application,~\gls{LPWAN} are suited to connect devices that only need to send a small amount of data over a long distance (100m to 15km or more). The key features in all of the  different~\gls{LPWAN} technologies will be a long range, and a low power consumption, for achieving this the data rate will also be low.
...
In short, all classes allow bidirectional communications. Class A support downlink communication after a uplink transmission, class B enables downlink scheduling and class C is always available for downlink communication, except when a device has to sent an uplink message. The last difference between classes is that A only support Unicast messages, whereas the others afford Unicast and Multicast.
...
\subsubsection{LoRa Devices Comparison} % (fold)
\label{sec:LoRa_table_sota}
In the following table~\ref{tab:Lora-Dev} a comparison between 4 devices using the LoRa technology.
...
Price & 54€ & 60€ & 15€ & 12€ \\
Technical : &  &  &  &  \\
Microcontroler & Esp32 & ATmega32U4 & Esp32 & \begin{tabular}[c]{@{}l@{}}ARM Cortex\\  M0+ Core\end{tabular} \\
...
The message payload size goes from zero (called "keep alive messages") until twelve bytes while operating in uplink. A twelve byte message is small but its enough to to transfer sensor data, the status of devices ,~\gls{GPS} coordinates or even application data as seen in figure~\ref{fig:SigFox_Payload_size}.\\
\begin{figure}[htbp]
  \centering
...
To solve this issue SIGFOX uses a random access feature. This feature consist in unsynchronized transmission between the network and the devices. In uplink operation the device emits a message on random frequency, that is later followed by two copies transmitted on different frequencies and time, while the base stations monitor all the spectrum and search for UNB signals. This feature also slightly surpass the  reliability problems occurring do  Sigfox lack of message arrival acknowledgement\cite{Mekki2019}.

Downlink messages have to be initiated by the end device, the frequency in use is the same as the first uplink message plus a known delta, there is a delay of twenty seconds between the first transmitted frame and a reception window this one only last for a maximum of twenty five seconds~\cite{SIGTECH}.
...
\subsubsection{Sigfox Devices Comparison} % (fold)
\label{sec:Sigfox_table_sota}
In the following table~\ref{tab:Sigfox-comparison} a comparison between 3 devices using the Sigfox technology is presented, where the price as well technical features are presented.\\

\begin{table}[htbp]
\begin{tabular}{@{}lllll@{}}
\toprule
Company & Pycom & STM & ON Semicondutor & Arduino \\ \midrule
Product & Fipy & B-L072Z-LRWAN1 & SFJK-API-1-GEVK & MKRFOX1200 \\
Price & 54€ & 62€ & 70€ & 33€ \\
\multicolumn{5}{l}{Technical:} \\
...
NB-IoT terminal comprises the sum of all end devices into the system. These terminals have access to the network as long as the correct SIM card is installed.\\The base stations refers to pre-existing nodes already been deployed by telecom operators. Usually these supports all three types of deployment modes shown before.\\Core network behaves like a bridge, enabling connections between base station and a cloud platform.\\The cloud platform can process various service, then forwards the  outputs to the vertical industry center whose function is up to the client or directly to the NB-IoT terminal.\\ Typically the Vertical Industry layer has~\gls{GUI}, for showing the data collected by the system, also it has control mechanisms for actuators or another device embedded into the terminal layer~\cite{Chen2017}.
\subsubsection{NB-IoT Decices Comparison} % (fold)
\label{sec:NB_table_sota}
In the following table a comparison between 3 devices using the NB-IoT technology.\\
...
\subsubsection{LTE-M Decices Comparison} % (fold)
\label{sec:LTE-M_Table_sota}
In the following table a comparison between 3 devices using the LTE-M technology.\\
\begin{table}[htbp]
\begin{tabular}{@{}lllll@{}}
\toprule
...
There are other LPWANs technologies, that are not present in the hardware utilized for this thesis, a brief overview is presented bellow.\newline\newline
\textbf{Weightless}\\
...
An analysis of related work shows the advancements in individual technologies and techniques for geolocation, but few cases that leverage multiple solutions.\\
...
%Many systems for  positioning over radio technologies have been proposed, based on techniques including RSSI [2, 3, 4, 5, 6, 7, 8], ToF [9, 10, 11, 12], or Time Difference on Arrival (TDoA) [13, 14]. Work has also been done to combine multiple ranging techniques into hybrid systems [15, 16]
